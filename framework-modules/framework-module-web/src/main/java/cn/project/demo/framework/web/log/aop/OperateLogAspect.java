package cn.project.demo.framework.web.log.aop;

import cn.hutool.core.exceptions.ExceptionUtil;
import cn.hutool.core.util.ArrayUtil;
import cn.hutool.core.util.StrUtil;
import cn.hutool.extra.servlet.ServletUtil;
import cn.project.demo.framework.common.exception.enums.GlobalErrorCodeConstants;
...
import cn.project.demo.framework.web.core.util.WebFrameworkUtils;
import cn.project.demo.framework.web.log.anno.OperateLog;
...
public class OperateLogAspect {
	...
	public Object around(ProceedingJoinPoint joinPoint, ApiOperation apiOperation) throws Throwable {
		...
		return aroundHandler(joinPoint, operateLog, apiOperation);
	}

	@Around("!@annotation(io.swagger.annotations.ApiOperation) && @annotation(operateLog)") // 兼容处理，只添加 @OperateLog 注解的情况
	public Object around(ProceedingJoinPoint joinPoint, OperateLog operateLog) throws Throwable {
		return aroundHandler(joinPoint, operateLog, null);
	}

	private Object aroundHandler(ProceedingJoinPoint joinPoint, OperateLog operateLog, ApiOperation apiOperation) throws Throwable {
		// 记录开始时间
		...
		try {
			...
			// 记录正常执行时的操作日志
			this.toSaveLog(joinPoint, operateLog, apiOperation, startTime, result, null);
			return result;
		} catch (Throwable exception) {
			this.toSaveLog(joinPoint, operateLog, apiOperation, startTime, null, exception);
			throw exception;
		} finally {
			clearThreadLocal();
		}
	...
	private void toSaveLog(ProceedingJoinPoint joinPoint, OperateLog operateLog, ApiOperation apiOperation,
		...
		try {
			...
			// 真正记录操作日志
			this.saveLog(joinPoint, operateLog, apiOperation, startTime, result, exception);
		} catch (Throwable ex) {
	...
	private void saveLog(ProceedingJoinPoint joinPoint, OperateLog operateLog, ApiOperation apiOperation,
	                     Date startTime, Object result, Throwable exception) {
	...
	private static void fillModuleFields(OperateLogCreateReqDTO operateLogDTO,
		...
		if (operateLogDTO.getType() == null) {
			...
			SystemOperateTypeEnum operateLogType = convertOperateLogType(requestMethod);
	...
	private static void fillMethodFields(OperateLogCreateReqDTO operateLogDTO,
		...
		if (result != null) {
			if (result instanceof CommonResult) {
				...
				operateLogDTO.setResultCode(GlobalErrorCodeConstants.SUCCESS.getCode());
			}
		}
		// （异常）处理 resultCode 和 resultMsg 字段
		if (exception != null) {
			operateLogDTO.setResultCode(GlobalErrorCodeConstants.INTERNAL_SERVER_ERROR.getCode());
	...
	private static SystemOperateTypeEnum convertOperateLogType(RequestMethod requestMethod) {
		if (requestMethod == null) {
			return null;
		}
		switch (requestMethod) {
			case GET:
				return SystemOperateTypeEnum.GET;
			case POST:
				return SystemOperateTypeEnum.CREATE;
			case PUT:
				return SystemOperateTypeEnum.UPDATE;
			case DELETE:
				return SystemOperateTypeEnum.DELETE;
			default:
				return SystemOperateTypeEnum.OTHER;
		}
	}
